...
static uint32_t const kPreferredSampleRate = 44100;
...
static ExampleCoreAudioContext *renderingContext;
static ExampleCoreAudioContext *capturingContext;
...
- (BOOL)startRendering:(nonnull TVIAudioDeviceContext)context {
    @synchronized(self) {
        NSAssert(renderingContext == NULL, @"Should not have any rendering context.");

        renderingContext = malloc(sizeof(ExampleCoreAudioContext));
        renderingContext->deviceContext = context;
        renderingContext->maxFramesPerBuffer = _renderingFormat.framesPerBuffer;
        ...
        renderingContext->expectedFramesPerBuffer = sessionFramesPerBuffer;

        NSAssert(self.audioUnit == NULL, @"The audio unit should not be created yet.");
        if (![self setupAudioUnit:renderingContext]) {
            free(renderingContext);
...
- (BOOL)stopRendering {
    ...
    @synchronized(self) {
        NSAssert(renderingContext != NULL, @"Should have a rendering context.");
        TVIAudioSessionDeactivated(renderingContext->deviceContext);
...
- (BOOL)startCapturing:(nonnull TVIAudioDeviceContext)context {
    @synchronized (self) {
        //NSAssert(capturingContext == NULL, @"Should not have any rendering context.");
        capturingContext = malloc(sizeof(ExampleCoreAudioContext));
        capturingContext->deviceContext = context;
        capturingContext->maxFramesPerBuffer = _renderingFormat.framesPerBuffer;
...
//    }
    ...
    for (int i = 0; i< audioBufferSizeInBytes; i++) {
        if (*(audioBuffer+i) != 0) {
            nonNullFound = true;
            break;
        }
...
- (void)handleAudioInterruption:(NSNotification *)notification {
    ...
    @synchronized(self) {
        ...
        TVIAudioDeviceContext context = renderingContext ? renderingContext->deviceContext : NULL;
        if (context) {
...
- (void)handleRouteChange:(NSNotification *)notification {
    ...
    switch (reason) {
        ...
        case AVAudioSessionRouteChangeReasonRouteConfigurationChange:
            ...
            @synchronized(self) {
                if (renderingContext) {
                    TVIAudioDeviceExecuteWorkerBlock(renderingContext->deviceContext, ^{
...
- (void)handleValidRouteChange {
    ...
    if (![activeFormat isEqual:_renderingFormat]) {
        ...
        @synchronized(self) {
            if (renderingContext) {
                TVIAudioDeviceFormatChanged(renderingContext->deviceContext);
            }
        }
    }
}
